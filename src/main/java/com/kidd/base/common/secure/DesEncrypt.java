...
public class DesEncrypt {
	...
	public byte[] Des(byte[] desKey, byte[] desData, int flag) {
		// 检测输入参数格式是否正确，错误直接返回空值（null）
		if ((desKey.length != 8) || (desData.length != 8)
				|| ((flag != 1) && (flag != 0))) {
			return null;
		}
		// 处理加密解密过程
		...
		keydata = readDataToInt(desKey);
		timedata = readDataToInt(desData);
		keyInitialize(keydata, keyArray);
		return encrypt(timedata, flags, keyArray);
	}
	...
	private byte[] encrypt(int[] timeData, int flag, int[][] keyarray) {
		...
		for (i = 0; i < 64; i++) {
			mip1[i] = m[IP_1[i] - 1]; // 进行IP-1运算
		}
		getEncryptInt(mip1, encrypt);
		// 返回加密数据
		return encrypt;
	}
	...
	private int loopF(int[] mM, int times, int flag, int[][] keyarray) {
		int i, j;
		int[] l0 = new int[32];
		int[] r0 = new int[32];
		int[] l1 = new int[32];
